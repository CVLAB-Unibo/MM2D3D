...
class VirtualKITTIBase(Dataset):
    ...
    proj_matrix = np.array(
        [[725, 0, 620.5], [0, 725, 187], [0, 0, 1]], dtype=np.float32
    )
...
class VirtualKITTISCN(VirtualKITTIBase):
    def __init__(
        ...
        if len(downsample) == 1:
            ...
            NotImplementedError(
                "Downsample must be either a tuple of (num_points,) or (min_points, max_points),"
    ...
    def __getitem__(self, index):
        ...
        intrinsics = self.proj_matrix.copy()
        ...
        seg_label[seg_label == 99] = len(self.label_mapping) - 1
        ...
        if self.camera_coords:
            points = points_cam_coords.copy()

        points_img = (self.proj_matrix @ points_cam_coords.T).T
        points_img = points_img[:, :2] / np.expand_dims(
            points_img[:, 2], axis=1
        )  # scale 2D points
        ...
        depth[img_indices[:, 0], img_indices[:, 1]] = points_cam_coords[:, 2]
...
def test_VirtualKITTISCN():
    from xmuda.data.utils.visualize import draw_bird_eye_view, draw_points_image_labels
    ...
    for i in 5 * [0]:
        ...
        draw_points_image_labels(
            img, img_indices, seg_label, color_palette_type="VirtualKITTI", point_size=3
        )
